...
mod sentry_tests {
    ...
    #[test]
    fn test_persistent_engine_cleanup_uncommitted() {
        use crate::wal::UncommittedInsert;
        let temp_dir = TempDir::new().unwrap();
        let mut engine = PersistentEngine::open(temp_dir.path()).unwrap();
        let inserts = vec![UncommittedInsert {
            relation_name: "nonexistent".to_string(),
            tuple_data: vec![],
        }];
        assert!(engine.undo_uncommitted_inserts(inserts).is_ok());
    }

    #[test]
    fn test_persistent_engine_wal_open_error() {
        let temp_dir = TempDir::new().unwrap();
        ...
        assert!(matches!(result.err().unwrap(), StorageError::Other(_)));
    }

    #[test]
    fn test_persistent_engine_txn_load_error() {
        let temp_dir = TempDir::new().unwrap();
        let mut engine = PersistentEngine::open(temp_dir.path()).unwrap();
        let result = engine.load_relation_for_txn("test", crate::wal::TransactionId::new(999));
        assert!(result.is_err());
        assert!(matches!(result.err().unwrap(), StorageError::Other(_)));
    }
}
