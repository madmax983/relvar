...
pub fn mpnn_layer(
    ...
    let messages = edges.join(&source_features)
    ...
    let aggregated = messages.summarize(
        &["node", "feature_id"],
    ...
    let weighted = agg_to_join.join(weights)
        .map_err(|e| DatabaseError::AlgebraError(e.to_string()))?;
    let multiplied = weighted.extend("out_value", ScalarType::Float, |t: &Tuple| {
    ...
    let final_features = multiplied.summarize(
        &["node", "out_feature_id"],
...
mod tests {
    ...
    fn test_mpnn_layer() {
        ...
        node_features.insert(tuple! { node: 3i64, feature_id: 1i64, value: 0.3f64 }).unwrap();
        ...
        weights.insert(tuple! { in_feature_id: 1i64, out_feature_id: 2i64, weight: 2.0f64 }).unwrap();
        ...
        let node2_tuple = result.restrict(|t| t.get_typed::<i64>("node").unwrap() == 2).tuples().next().unwrap().clone();
        assert_eq!(node2_tuple.get_typed::<i64>("feature_id").unwrap(), 2);
        assert_eq!(node2_tuple.get_typed::<f64>("value").unwrap(), 1.0);

        let node3_tuple = result.restrict(|t| t.get_typed::<i64>("node").unwrap() == 3).tuples().next().unwrap().clone();
