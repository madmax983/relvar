...
fn validate_division_compatibility(
    ...
    for attr_name in divisor_heading.attribute_names() {
        ...
        match dividend_heading.get_attribute_type(attr_name) {
            None => {
                return Err(DatabaseError::AttributeNotFound(attr_name.clone(), "dividend".to_string()));
            }
            Some(dividend_type) => {
...
fn compute_remainder_attributes(
    ...
    if remainder_attrs.is_empty() {
        return Err(DatabaseError::AlgebraError("Divisor heading cannot equal dividend heading (no remainder attributes)".to_string()));
    }
