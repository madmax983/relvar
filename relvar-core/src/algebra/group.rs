...
impl Relation {
    ...
    /// - [`DatabaseError::AlgebraError(msg) if msg == "No attributes specified for grouping"`] - Empty attributes list
    /// - [`DatabaseError::AlgebraError(msg) if msg == "All attributes cannot be grouped (need at least one non-grouped attribute)"`] - No grouping key attributes remain
...
fn validate_group_request(
    ...
    if attrs_to_group.len() == relation.relation_type().degree() {
        return Err(DatabaseError::AlgebraError(
            "All attributes cannot be grouped (need at least one non-grouped attribute)"
                .to_string(),
        ));
    }
...
mod tests {
    ...
    fn test_group_all_attributes_fails() {
        ...
        assert!(matches!(
            result.unwrap_err(),
            DatabaseError::AlgebraError(msg) if msg == "All attributes cannot be grouped (need at least one non-grouped attribute)"
        ));
    }
