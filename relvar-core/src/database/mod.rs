...
impl<E: StorageEngine> Database<E> {
    ...
    pub fn define_virtual_relvar(
        ...
        self.virtual_relvars.insert(
            name.to_string(),
            virtual_relvar::VirtualRelvarDefinition {
                name: name.to_string(),
                relation_type,
                evaluator,
            },
...
mod tests {
    ...
    fn test_virtual_relvar_immutability_enforcement() {
        ...
        // like insert() inside the evaluator because it receives &Database<InMemoryEngine>, not &mut Database.
        db.define_virtual_relvar(
            "SAFE_VIEW",
            test_rel_type(),
            |db: &Database<InMemoryEngine>| {
