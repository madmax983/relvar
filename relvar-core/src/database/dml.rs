...
mod tests {
    ...
    fn get_test_relation() -> Relation {
        ...
        rel.insert(tuple! { id: 2i64 }).unwrap();
        rel.insert(tuple! { id: 3i64 }).unwrap();
        rel
    }
    ...
    fn test_compute_relation_after_delete() {
        ...
        assert!(new_rel.contains(&tuple! { id: 1i64 }));
    }
    ...
    fn test_compute_relation_after_update() {
        ...
        assert!(new_rel.contains(&tuple! { id: 1i64 }));
        assert!(new_rel.contains(&tuple! { id: 20i64 }));
        assert!(new_rel.contains(&tuple! { id: 3i64 }));
    }
    ...
    fn test_compute_relation_after_update_type_mismatch() {
        ...
        let result = compute_relation_after_update(
            ...
            |_| {
                // Wrong type!
                let tuple_type = TupleType::new().with_attribute("id", ScalarType::String);
                let mut values = std::collections::BTreeMap::new();
                values.insert("id".to_string(), ScalarValue::String("wrong".to_string()));
                Tuple::new(std::sync::Arc::new(tuple_type), values).unwrap()
            },
        );
