...
use crate::constraints::{ForeignKeyConstraints, ForeignKey};
use crate::constraints::{AttributeConstraints, TypeConstraint};
use crate::constraints::{CheckConstraints, CheckConstraint, ConstraintExpression, CmpOp, ValueOrRef};
...
fn test_database_integrity_getters() {
    ...
    assert_eq!(constraints.primary_key().unwrap().attributes(), &vec!["id".to_string()]);
...
fn test_database_integrity_type_and_check() {
    ...
    let checks = CheckConstraints::new()
