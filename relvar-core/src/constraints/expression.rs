...
mod like_tests {
    ...
    fn test_like_wildcard_percent() {
        // Matches "Al" followed by anything
        ...
        assert!(expr.evaluate(&tuple! { name: "Al" }).unwrap()); // Matches empty suffix
    ...
    fn test_like_wildcard_underscore() {
        // Matches "B" followed by exactly one char, followed by "b"
        let expr = ConstraintExpression::Like("name".to_string(), "B_b".to_string());

        assert!(expr.evaluate(&tuple! { name: "Bob" }).unwrap());
        assert!(expr.evaluate(&tuple! { name: "Bib" }).unwrap());

        assert!(!expr.evaluate(&tuple! { name: "Boob" }).unwrap()); // Too long
        assert!(!expr.evaluate(&tuple! { name: "Bb" }).unwrap()); // Too short
    }

    #[test]
    fn test_like_complex_pattern() {
        // "A%c_" matches start with A, anything in middle, ends with c then one char
        let expr = ConstraintExpression::Like("code".to_string(), "A%c_".to_string());

        assert!(expr.evaluate(&tuple! { code: "Abcd" }).unwrap());
        assert!(expr.evaluate(&tuple! { code: "Ac1" }).unwrap());

        assert!(!expr.evaluate(&tuple! { code: "Abc" }).unwrap()); // Missing last char
        assert!(!expr.evaluate(&tuple! { code: "Bbc1" }).unwrap()); // Doesn't start with A
    }

    #[test]
    fn test_like_empty_string_and_pattern() {
        let empty_pattern = ConstraintExpression::Like("text".to_string(), "".to_string());

        assert!(empty_pattern.evaluate(&tuple! { text: "" }).unwrap());
        assert!(!empty_pattern.evaluate(&tuple! { text: "a" }).unwrap());

        let percent_pattern = ConstraintExpression::Like("text".to_string(), "%".to_string());
        assert!(percent_pattern.evaluate(&tuple! { text: "" }).unwrap());
        assert!(percent_pattern.evaluate(&tuple! { text: "abc" }).unwrap());
    }

    #[test]
    fn test_like_unicode() {
        let expr = ConstraintExpression::Like("word".to_string(), "s_n".to_string());

        // "schön" -> s, c, h, ö, n (5 chars). Pattern "s_n" expects 3 chars.
        // "sön" -> s, ö, n (3 chars). Should match if _ matches one Unicode char.
        assert!(expr.evaluate(&tuple! { word: "sön" }).unwrap());

        // "son"
        assert!(expr.evaluate(&tuple! { word: "son" }).unwrap());

        let emoji_expr = ConstraintExpression::Like("msg".to_string(), "Hello % 🌍".to_string());
        assert!(
            emoji_expr
                .evaluate(&tuple! { msg: "Hello world 🌍" })
                .unwrap()
        );
    }
    ...
    fn test_like_type_mismatch() {
        let expr = ConstraintExpression::Like("age".to_string(), "3%".to_string());

        // Attempt to run LIKE on an integer
        let tuple = tuple! { age: 30i64 };
        let result = expr.evaluate(&tuple);
