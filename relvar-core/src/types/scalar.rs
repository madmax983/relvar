...
/// let result = ScalarValue::select(&emp_id_type, ScalarValue::String("Not an Int".into()));
///
...
pub enum ScalarType {
    ...
    /// let id_val = ScalarValue::select(&emp_id_type, ScalarValue::Int(100)).unwrap();
...
mod tests {
    ...
    fn test_builtin_selector_validates_type() {
        ...
        let result = ScalarValue::select(&int_type, string_value);
        assert!(result.is_err());

        // Should accept correct type
        let int_value = ScalarValue::Int(42);
        let result = ScalarValue::select(&int_type, int_value.clone());
        assert!(result.is_ok());
