...
///
/// ```
/// use relvar::{Database, InMemoryEngine, visualizer::to_dot};
///
/// let mut db = Database::new(InMemoryEngine::new());
/// // ... setup schema ...
///
///
/// let dot_code = to_dot(&db);
/// println!("{}", dot_code);
/// ```
pub fn to_dot<E: StorageEngine>(db: &Database<E>) -> String {
    ...
    for name in &relvars {
        ...
        if let Ok(relation_type) = db.get_relvar_type(name) {
