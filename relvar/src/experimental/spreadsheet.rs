//! A relational spreadsheet modeling and evaluation engine.
//!
//! This module provides a way to represent a spreadsheet using relations:
//! one for raw values and another for formulas. It evaluates formulas
//! using relational operations (joins and extensions) until a fixpoint
//! is reached and all cells are resolved.
...
impl Spreadsheet {
    ...
    ///
    /// // B1 is resolved to 30.0
    /// let b1_tuple = result.tuples().find(|t| t.get_typed::<String>("id").unwrap() == "B1").unwrap();
    /// assert_eq!(b1_tuple.get_typed::<f64>("val").unwrap(), 30.0);
    /// ```
