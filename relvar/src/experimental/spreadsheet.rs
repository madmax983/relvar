...
///
/// // 2. Create the formulas relation
...
impl Spreadsheet {
    ...
    /// use relvar::experimental::spreadsheet::Spreadsheet;
    ///
    /// let val_type = RelationType::new(TupleType::new()
    ///     .with_attribute("id", ScalarType::String)
    ///     .with_attribute("val", ScalarType::Float));
    /// let form_type = RelationType::new(TupleType::new()
    ///     .with_attribute("id", ScalarType::String)
    ///     .with_attribute("op", ScalarType::String)
    ///     .with_attribute("arg1", ScalarType::String)
    ///     .with_attribute("arg2", ScalarType::String));
    ///
    /// let values = Relation::new(val_type);
    /// let formulas = Relation::new(form_type);
    ///
    ...
    /// values.insert(tuple! { id: "X".to_string(), val: 5.0f64 }).unwrap();
    /// values.insert(tuple! { id: "Y".to_string(), val: 3.0f64 }).unwrap();
    ///
    /// let form_type = RelationType::new(TupleType::new()
    ///     .with_attribute("id", ScalarType::String)
    ///     .with_attribute("op", ScalarType::String)
    ...
    /// // Z = X * Y
    /// formulas.insert(tuple! {
    ///     id: "Z".to_string(), op: "MUL".to_string(), arg1: "X".to_string(), arg2: "Y".to_string()
    /// }).unwrap();
    ///
    /// let sheet = Spreadsheet::new(values, formulas);
    /// let evaluated = sheet.evaluate().unwrap();
    ///
    /// assert_eq!(evaluated.cardinality(), 3); // X, Y, Z
    /// ```
