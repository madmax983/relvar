...
impl NeuralNetwork {
    ...
    pub fn forward_layer(&mut self, current_layer_idx: i64) -> Result<(), DatabaseError> {
        let with_products = self.compute_weighted_inputs(current_layer_idx)?;
        let sum_products = self.summarize_pre_activations(&with_products, current_layer_idx)?;
    ...
    fn compute_weighted_inputs(&self, current_layer_idx: i64) -> Result<Relation, DatabaseError> {
    ...
    fn apply_biases_and_activation(
        ...
        joined_biases
            ...
            .map_err(|e| DatabaseError::AlgebraError(e.to_string()))
            .map(|r| r.project(&["layer", "node", "val"]))
    }
