...
impl PhysicsEngine {
    ...
    fn compute_pairwise_forces(&self) -> Result<Relation, DatabaseError> {
        ...
        let g = self.g;
        interactions
            .extend("fx", ScalarType::Float, create_force_calculator(g, true))
            .map_err(|e| DatabaseError::AlgebraError(e.to_string()))?
            .extend("fy", ScalarType::Float, create_force_calculator(g, false))
            .map_err(|e| DatabaseError::AlgebraError(e.to_string()))
    }
...
fn create_force_calculator(
    g: f64,
    is_x: bool,
) -> impl Fn(&relvar_core::values::Tuple) -> ScalarValue {
    move |t| {
        ...
        let component = if is_x {
            f * (dx / dist)
        } else {
            f * (dy / dist)
        };

        ScalarValue::Float(component)
    }
}
