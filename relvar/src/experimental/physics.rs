...
impl PhysicsEngine {
    ...
    fn compute_pairwise_forces(&self) -> Result<Relation, DatabaseError> {
        ...
        let compute_force = move |t: &relvar_core::Tuple, is_x: bool| -> ScalarValue {
            ...
            let component = if is_x { dx } else { dy };
            let force_component = f * (component / dist);

            ScalarValue::Float(force_component)
        };

        interactions
            .extend("fx", ScalarType::Float, move |t| compute_force(t, true))
            .map_err(|e| DatabaseError::AlgebraError(e.to_string()))?
            .extend("fy", ScalarType::Float, move |t| compute_force(t, false))
            .map_err(|e| DatabaseError::AlgebraError(e.to_string()))
    }
