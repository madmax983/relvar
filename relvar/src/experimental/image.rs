...
pub fn save(relation: &Relation) -> (usize, usize, Vec<u8>) {
    ...
    let width = usize::try_from(max_x.abs_diff(min_x).saturating_add(1)).unwrap_or(0);
    let height = usize::try_from(max_y.abs_diff(min_y).saturating_add(1)).unwrap_or(0);
    ...
    for tuple in relation.tuples() {
        ...
        let img_x = usize::try_from(x.abs_diff(min_x)).unwrap_or(0);
        let img_y = usize::try_from(y.abs_diff(min_y)).unwrap_or(0);
