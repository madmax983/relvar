...
impl MockRelation {
    ...
    /// # Arguments
    ///
    /// * `relation_type` - The schema of the relation to generate.
    /// * `count` - The number of tuples to generate. Duplicate tuples may reduce the final cardinality.
    /// * `seed` - An optional random seed for deterministic generation.
    ///
    /// # Examples
    ///
    /// ```
