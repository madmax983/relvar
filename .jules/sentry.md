...
## 2026-06-19 - Database Missing Error Branch Tests
**Learning:** Found several untested code paths regarding the failure branches of `update` and `delete` in `relvar-core/src/database/data.rs` and `relvar-core/src/database/dml.rs`. Specifically, the tuple format validation failures when calculating state replacements during DML execution and FK bulk validations failed to exercise the internal iterators fully. Additionally, several getters and setters in `integrity.rs` were entirely missing test cases or documentation validation.
**Action:** Wrote targeted coverage tests for `data.rs` constraint verification, `dml.rs` error mappings, and comprehensive coverage of `integrity.rs` and `schema.rs` constraints tracking to properly hit early `?` return statements. Always ensure unit testing mirrors public interface tests for `database` layers handling error boundaries.
