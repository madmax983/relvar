...
## 2026-05-20 - Storage Heap Extraction & Algebra Into Error Coverage
**Learning:** Discovered untested error boundaries in `relvar-storage` for extracting raw tuples and slotted tuples (`extract_tuples_from_slots`, `extract_all_tuples`) within heap file manipulation, which could hide serialization and memory slicing bounds errors. Additionally, `relvar-core`'s algebra `_into` fast-paths (`extend_into`, `union_into`, `intersect_into`, `difference_into`) and `Delta::compose` lacked comprehensive coverage for type mismatch error propagation.
**Action:** When creating high-performance internal memory utilities or consuming `_into` equivalents of standard algebra methods, always ensure explicit tests exist to trigger their unique bounds-checking and error-propagation mappings, as they often deviate from their referencing counterparts.
