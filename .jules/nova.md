...
**Concept:** Modeled a Reinforcement Learning Value Iteration algorithm using pure relational algebra. States, Actions, Transitions, and Rewards are relations. The Bellman equation is evaluated declaratively via Joins, Extensions, and Aggregations (Sum, Max).
**Fate:** Merged
**Lesson:** Relational algebra is surprisingly adept at dynamic programming and optimal control! The Bellman update compiles cleanly into a Join (to match transitions to current values), an Extend (for discounting), and Summarize operations to compute expected future value and maximum Q-values without procedural loops.
