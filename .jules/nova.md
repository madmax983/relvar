...
**Concept:** Modeled a Turing Machine purely with relational algebra. The tape, head position/state, and transition table are relations. The Turing Machine executes via Joins to match state/symbol, Differences/Unions to modify tape and head state, simulating standard operations entirely within a relational engine context.
**Fate:** Merged
**Lesson:** Universal computation is possible in Relational Algebra! By representing the tape sparsely and joining it with transition relations, each step of the machine is just a relational query mapping `(state, read_symbol)` to `(next_state, write_symbol, direction)`.
