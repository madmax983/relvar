...
**Concept:** Modeled a Quantum Circuit Simulator purely using relational algebra. Quantum states are represented as relations of probability amplitudes, and quantum gates are represented as relations. Gate application maps to joining state vectors with gate matrices and aggregating (Matrix Multiplication). Multi-qubit gates are computed via relational Kronecker products (Cross Joins).
**Fate:** TBD
**Lesson:** Relational engines can model quantum mechanics! Matrix operations and tensor products elegantly compile down to relational joins, extensions, and aggregations, demonstrating that database queries can simulate state transitions.
