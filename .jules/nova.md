...
## Relational CRDTs (OR-Set)
**Concept:** Modeled an Observed-Remove Set (OR-Set) CRDT using purely relational algebra. Represents `adds` and `removes` as relations with unique tags. Evaluating the current state is performed using relational Difference and Project, and merging two states is performed using relational Union.
**Fate:** Merged
**Lesson:** Relational algebra maps exceptionally well to operation-based CRDTs! Because standard relational unions natively handle deduplication, merging states is completely conflict-free. Relational difference gracefully resolves tombstones, proving that robust distributed data types can be built with pure queries.
