...
## Relational K-Nearest Neighbors (KNN)\n**Concept:** Implemented `KnnClassifier` using purely relational algebra. Distances are computed via `Extend`, Top-K neighbors are ranked using a declarative counting pattern (Join + Restrict + Summarize), and the majority vote is found via another Top-1 declarative pattern.\n**Fate:** Merged\n**Lesson:** Relational algebra is expressive enough to implement lazy learning algorithms like K-Nearest Neighbors entirely natively. By using Theta-Joins to compute strictly smaller elements and summarizing, we can achieve ranking (Top-N queries) and voting natively within the algebra without procedural constructs!
