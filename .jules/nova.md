...
**Concept:** Modeled the constraint propagation of the Wave Function Collapse algorithm using purely relational algebra. Given a domain of potential tiles and allowed adjacencies, it iteratively filters out invalid tiles by using cross joins (with directions), joining with rules, and taking the set difference to remove invalid candidates.
**Fate:** Merged
**Lesson:** Constraint satisfaction algorithms like WFC map beautifully to pure relational operations. Operations like calculating all active neighbor checks, verifying valid adjacencies via joins, and reducing the candidate domain through difference perfectly encapsulate relational set theory in action.
