...
## Relational Quantum Simulator
**Concept:** Modeled a quantum computer simulator using purely relational algebra. The quantum state is a sparse relation `(basis, amplitude_real, amplitude_imag)`. Quantum gates (Hadamard, Pauli-X, CNOT) are applied via `Join` with a gate transition relation, followed by `Summarize` to compute quantum interference by adding complex amplitudes.
**Fate:** TBD
**Lesson:** Relational algebra maps beautifully to quantum mechanics! The sparse state vector maps perfectly to a relation, gate applications are just joins and extensions (complex multiplication), and quantum interference is simply aggregation (`Summarize`). By chaining joins and aggregates, we can declaratively simulate quantum circuits!
