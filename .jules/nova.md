...
## Relational Turing Machine
**Concept:** Modeled a Turing Machine where the tape and transitions are purely relational. Evaluates machine progression directly using purely declarative queries via `Restrict`, `Difference`, `Insert` inside a `while` loop stepping structure until a halt is reached.
**Fate:** Merged
**Lesson:** Turing machines can be nicely adapted to a declarative model, where states and transition matrices easily represent tuples and relational query logic can compute arbitrary functions!
