...
## Relational Wordle
**Concept:** Modeled Wordle game logic using pure relational algebra. Words are represented as relations of `(position: Int, letter: String)`. Guess evaluation is performed by chained operations. Exact matches are found via natural joins, and partial matches are resolved by using a Theta Join to rank guess letters from left to right and joining with a summarized frequency relation of the target letters. Misses are found by taking differences.
**Fate:** TBD
**Lesson:** Wordle's left-to-right duplicate tie-breaking rules can be elegantly modeled in pure set algebra by computing a positional rank via Theta Join counting strictly earlier matching letters, without any procedural loops!
