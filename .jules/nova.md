...
## Relational Vector Database (Similarity Search)
**Concept:** Modeled a Vector Search database using purely relational algebra. Vectors and their components are represented as relations. Similarity search (e.g., dot product or cosine similarity) is computed via Cartesian Product (Join), Extend (multiplication of components), and Summarize (Summing over components) without introducing any specialized index structures.
**Fate:** TBD
**Lesson:** TBD
