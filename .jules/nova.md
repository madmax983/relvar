...
**Concept:** Modeled a Conflict-free Replicated Data Type (OR-Set) purely using relational algebra. Addition and removal are represented as `adds` and `removes` relations. Set membership is evaluated via relation difference, and merging replicas is natively handled via relational unions.
**Fate:** TBD
**Lesson:** TBD
