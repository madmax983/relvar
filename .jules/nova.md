...
## Relational CA Sequencer
**Concept:** Implemented a Cellular Automaton Sequencer using pure relational algebra. Connects `cellular_automaton::CellularAutomaton` and `synth::Synth`. At each CA generation, alive cells are converted into oscillators (pitch based on y-coordinate) via `Extend`, synthesized via `Join` and `Summarize`, and accumulated into a master audio timeline via `Union`.
**Fate:** TBD
**Lesson:** TBD
