...
## Relational Turing Machine
**Concept:** Modeled a Turing Machine purely using relational algebra. The `Tape`, `State`, and `Rules` are represented as relations. The machine steps are computed via `join`, `extend`, `difference`, and `union` operations, resolving the next state and tape modifications declaratively.
**Fate:** Merged
**Lesson:** Relational algebra is Turing Complete! By continually computing joins and extensions between the state, tape, and transition rules, we can evaluate any computable function declaratively.
