...
## Relational Turing Machine
**Concept:** Modeled a Relational Turing Machine purely with relational algebra. Transitions, tape, and head are relations. A machine step is evaluated through joining head with tape, union with blank symbol read, joining with transitions, extending next position, and updating tape through semidifference and union.
**Fate:** Merged
**Lesson:** Relational algebra is Turing-complete! We can build a full Turing Machine where steps are simply declarative relational database queries mapping current state to the next state!
