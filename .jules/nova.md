...
**Concept:** Modeled a Build System (like Make/Ninja) using pure relational algebra. Dependencies are an edges relation, and file timestamps are a relation. Stale targets are resolved by finding files older than their dependencies via transitive closure (`tclose`), relational joins, and restrictions (`restrict`).
**Fate:** Merged
**Lesson:** Relational algebra gracefully handles dependency graphs. Transitive closure effortlessly computes all indirect dependencies, and joining with file timestamps lets us simply restrict to find stale files. It turns a graph traversal into a clean query!
