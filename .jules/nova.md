...
**Concept:** Modeled a Quantum State Vector and Quantum Gates purely using relational algebra. Qubits are attributes in a relation, mapping to complex amplitudes. Applying a 1-qubit gate (like Hadamard) translates directly into a relational join (tensor product) and summarization (matrix multiplication) over the state vector.
**Fate:** Merged
**Lesson:** Relational engines can simulate quantum circuits! By representing superposition as relation tuples with complex amplitudes, tensor products map elegantly to Natural Joins, and state collapse/measurement aligns with Relational Restrictions.
