...
## Relational Turing Machine
**Concept:** Modeled a Turing Machine purely with relational algebra. The tape, state, and transitions are relations. The machine steps are evaluated using `join` to find transitions, `extend` to calculate head movement, `semidifference` to erase the old symbol, and `union` to write the new symbol.
**Fate:** TBD
**Lesson:** Relational algebra is Turing-complete! We can evaluate any computable function purely via chained relation operations.
