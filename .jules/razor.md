...
## [Reduction]
**Bloat:** `QueryExecutor` trait. It was a "One-Time Trait" implemented only by `Database`, adding unnecessary abstraction and indirection for speculative "Future Proofing" of View execution.
**Cut:** Removed `QueryExecutor` trait, made `VirtualRelvarDefinition` generic over `StorageEngine`, and updated `Query::execute` to depend directly on `Database`.
**Saved:** Removed 1 file (`traits.rs`), removed ~20 lines of code, reduced cognitive load by removing an unnecessary abstraction layer.
