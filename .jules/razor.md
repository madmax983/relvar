...
## [Reduction]
**Bloat:** `QueryExecutor` trait. A "One-Time Trait" implemented only by `Database`, adding unnecessary abstraction layer between `Query` and `Database`.
**Cut:** Removed `QueryExecutor` trait, made `VirtualRelvarDefinition` generic over `StorageEngine`, and updated `Query::execute` to take `&Database<E>`.
**Saved:** Removed 1 file (`traits.rs`), simplified `Database` implementation, made dependencies explicit.
