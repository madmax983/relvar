...
## 2026-07-30 - Refactor God Function in Physics
**Learning:** The `compute_pairwise_forces` function in `relvar/src/experimental/physics.rs` was a God Function that combined renaming and cross-joining particles, filtering self-interactions, and extending attributes with deeply nested mathematical operations.
**Action:** Extracted the phases into private helper functions (`generate_particle_pairs`, `filter_self_interactions`, `calculate_pairwise_forces`, and `calculate_force_component`) to flatten the structure and drastically improve readability.
