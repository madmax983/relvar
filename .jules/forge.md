...
## 2024-08-17 - [Iterator Chains for complex logic]
**Learning:** Refactoring deeply nested imperative code that maintains multiple variables into iterator chains can dramatically improve code readability and convey intent clearly.
**Action:** When finding complex manual loops maintaining state variables, replace them with iterator pipelines like `.filter_map()`, `.retain()`, and `.collect()` to make data flow explicitly clear.
