...
## 2026-07-24 - Extract God Function in Ungroup
**Learning:** The `compute_ungrouped_tuples` function in `relvar-core/src/algebra/group.rs` was a "God Function" (55 lines) that mixed cardinality calculation, extracting relation-valued attributes (RVAs), computing base attributes, and combining them. This made the logical flow hard to follow.
**Action:** Applied the "Three-Phase Operator" pattern by extracting `calculate_total_capacity`, `extract_rva_relation`, and `compute_base_values` into separate helper functions. This drastically improved readability without altering logic or performance characteristics.
