...
## 2024-05-19 - Fix tools module visibility and unused imports
**Learning:** Marking an internal module like `tools` as `pub(crate)` breaks integration tests in the `tests/` directory which are considered external to the crate and need `pub` access.
**Action:** Always verify if a module is used by external tests before restricting its visibility to `pub(crate)`, and proactively remove unused imports to avoid `clippy::unused_imports`.
