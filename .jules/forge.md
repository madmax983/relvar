...
## 2026-07-16 - Extract God Function in Physics pairwise force computation
**Learning:** `relvar/src/experimental/physics.rs` contained a "God Function" `compute_pairwise_forces` (81 lines) which duplicated complex mathematics within closures to compute `fx` and `fy`.
**Action:** Applied the 'Three-Phase Operator' pattern by extracting the closure logic into a private higher-order helper function `create_force_calculator` that takes in `g: f64` and `is_x: bool` to prevent duplication and simplify the `extend` chain.
