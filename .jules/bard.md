...
**Confusion:** The experimental `Spreadsheet` module had placeholder examples for its `new` and `evaluate` methods, and the `Spreadsheet` struct lacked an example. This made it difficult to understand how to construct the values and formulas relations, or how the fixpoint evaluation worked.
**Clarification:** Added comprehensive executable `/// # Examples` doc-tests for `Spreadsheet`, `new`, and `evaluate` to clearly demonstrate how to model a spreadsheet and resolve formulas purely using relational operations.
