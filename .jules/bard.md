...
**Confusion:** The experimental `Spreadsheet` module had a placeholder example and lacked executable doc-tests for the main struct and its methods (`new` and `evaluate`). This lack of examples made it confusing to understand how to initialize the values and formulas relations and how to evaluate the fixpoint query.
**Clarification:** Replaced the placeholder and added comprehensive executable `/// # Examples` doc-tests for `Spreadsheet` and all its methods to clearly demonstrate how to model a spreadsheet and evaluate queries iteratively.
