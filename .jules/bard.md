...
**Confusion:** The experimental `Spreadsheet` module had placeholder doc-tests for the main struct and its methods (`new` and `evaluate`). This lack of examples made it confusing to understand how to initialize the values and formulas relations, or execute the fixpoint evaluation algorithm.
**Clarification:** Added executable `/// # Examples` doc-tests for `Spreadsheet` and its methods, demonstrating how to construct a simple spreadsheet and how to execute the fixpoint algorithm to resolve formulas.
