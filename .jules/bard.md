...
## 2024-05-18 - Documenting Spreadsheet Evaluator
**Confusion:** The experimental `Spreadsheet` engine had placeholder examples for its main struct and methods, leaving it unclear how it evaluates interconnected cells purely using relational joins and fixpoints.
**Clarification:** Replaced the placeholder and added executable `/// # Examples` doc-tests for the `Spreadsheet` struct and `new`/`evaluate` methods to explicitly demonstrate how to define values/formulas and resolve them.
