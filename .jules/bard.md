...
**Confusion:** The experimental `Spreadsheet` module had a placeholder example for the struct and lacked executable doc-tests for its methods (`new` and `evaluate`), making it difficult to understand how to construct the raw values and formulas relations or how the evaluation runs to fixpoint.
**Clarification:** Replaced the placeholder and added comprehensive executable `/// # Examples` doc-tests for `Spreadsheet` and its methods to clearly demonstrate how to model a spreadsheet and evaluate formulas declaratively.
