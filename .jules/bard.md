...
**Confusion:** The experimental `Spreadsheet` had a placeholder example for the struct and lacked executable doc-tests for its methods (`new` and `evaluate`), making it difficult to understand how to construct the formulas, and evaluate spreadsheet values.
**Clarification:** Replaced the placeholder and added comprehensive executable `/// # Examples` doc-tests for `Spreadsheet` and its methods to clearly demonstrate how to evaluate formulas and construct relations.
