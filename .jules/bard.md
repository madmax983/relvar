...
**Confusion:** The experimental `Spreadsheet` module lacked executable doc-tests (`/// # Examples` blocks) for its struct and methods (`new` and `evaluate`). This made it difficult for users to understand how to instantiate the values and formulas relations and run the fixpoint evaluation.
**Clarification:** Added comprehensive executable doc-tests to `Spreadsheet`, `new`, and `evaluate` that demonstrate how to construct the necessary relations and resolve formulas correctly.
