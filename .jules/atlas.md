...
## 2025-05-XX - Struct Field Encapsulation Cleanup
**Tangle:** Several structs across the codebase (`AnalysisResult`, `RecoveryResult`, `TransactionSnapshot`, `VersionMetadata`, `RelationMetadata` in `relvar-storage`, and `InMemorySnapshot`, `Aggregation`, `Delta`, `ScalarTypeError` in `relvar-core`, along with experimental structs) had `pub` fields, exposing internal implementation details and violating the principle of encapsulation ("Leaky Abstractions").
**Blueprint:** Changed the visibility of these fields from `pub` to `pub(crate)` to hide internal details while maintaining accessibility within their respective crates. Note: `RelationMetadata` in `relvar-core/src/storage_engine/mod.rs` was left as `pub` because it is part of the `StorageEngine` trait contract implemented by external crates (e.g., `relvar-storage`).
