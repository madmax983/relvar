...
## 2026-03-10 - HeapFile Integer Overflow in Slot Entries
**Threat:**  contained integer overflow vulnerabilities during tuple extraction where `offset + length` of slot entries was used. An attacker supplying a maliciously crafted page with carefully chosen `offset` and `length` values could wrap the pointer arithmetic back around on 32-bit platforms, tricking the length check `offset + length > data.len()` to evaluate to false and cause an out-of-bounds slice access panic or memory access violation.
**Defense:** Patched all usages of `offset + length` and `start + length` with `checked_add(length).ok_or_else(...)` in `relvar-storage/src/storage/heap.rs` including in the core slot iteration block, ensuring safe explicit bound checks.

## 2026-03-10 - HeapFile Integer Overflow in Slot Entries
**Threat:** `relvar-storage/src/storage/heap.rs` contained integer overflow vulnerabilities during tuple extraction where `offset + length` of slot entries was used. An attacker supplying a maliciously crafted page with carefully chosen `offset` and `length` values could wrap the pointer arithmetic back around on 32-bit platforms, tricking the length check `offset + length > data.len()` to evaluate to false and cause an out-of-bounds slice access panic or memory access violation.
**Defense:** Patched all usages of `offset + length` and `start + length` with `checked_add(length).ok_or_else(...)` in `relvar-storage/src/storage/heap.rs` including in the core slot iteration block, ensuring safe explicit bound checks.
