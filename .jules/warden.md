...
## 2026-05-25 - [Update vulnerable dependencies]
**Threat:** The dependency `crossbeam-epoch` (v0.9.18) had a known vulnerability (RUSTSEC-2026-0204: Invalid pointer dereference in `fmt::Pointer` impl) and `anyhow` (v1.0.102) had a known vulnerability (RUSTSEC-2026-0190: Unsoundness in `Error::downcast_mut()`).
**Defense:** Updated `crossbeam-epoch` to v0.9.20 and `anyhow` to v1.0.104 via `cargo update`.
